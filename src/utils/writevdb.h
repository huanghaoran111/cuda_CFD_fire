...
void VDBWriter::AddGridImpl<T, N>::operator()() const
{
    ...
    openvdb::tools::Dense<typename GridT::ValueType> dense(openvdb::Coord(sizex, sizey, sizez), openvdb::Coord(minx, miny, minz));
    for(uint32_t z = 0; z < sizez; ++z)
    {
        for(uint32_t y = 0; y < sizey; ++y)
        {
            for(uint32_t x = 0; x < sizex; ++x)
            {
                ...
                dense.setValue(x, y, z, help_make_vec<typename GridT::ValueType>(ptr, std::make_index_sequence<N>{}));
            }
        }
    }
